...
# from .models import Section, SectionState, Story, StoryState
...
#     for epic in Section.objects.filter(story__id__in=story_ids).distinct():
#         epic.update_state()
...
#     for pk in epic_ids:
#         try:
#             epic = Section.objects.get(pk=pk)
...
#     Section.objects.filter(id__in=epic_ids).delete()


# @app.task(ignore_result=True)
# def epic_set_owner(epic_ids, user_id):
#     Section.objects.filter(id__in=epic_ids).update(owner=user_id)
...
#     Section.objects.filter(id__in=epic_ids).update(state=state)

#     for epic in Section.objects.filter(id__in=epic_ids):
#         epic.update_state()
...
#     try:
#         epic = Section.objects.get(pk=epic_id)
#     except Section.DoesNotExist:
#         return
